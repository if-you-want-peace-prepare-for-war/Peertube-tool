...
#
#
# Do your main configureion in your local ~/yt-dlp.conf, like setting
...
# Set ffmpeg path
ffmpeg="$(command -v $HOME/.local/bin/ffmpeg || command -v /usr/local/bin/ffmpeg || command -v /usr/bin/ffmpeg)"
if [ -z "$ffmpeg" ]; then
    echo "FFmpeg not found. Please install it to use this script."
    exit 1
fi
...
yt_dlp=$(command -v /usr/local/bin/yt-dlp || command -v /usr/bin/yt-dlp)
if [[ -z "$yt_dlp" ]]; then
...
sub_langs="en-nP7*,fr-*,da-*,de-*,pt-*,es-*,en-nP7-2PuUl7o,fr-rSJfcHgCuhE,de-hrwxFetOmHM,pt-BR-ykZLUhi1BHQ,es-y1SAwy5xd4g"
